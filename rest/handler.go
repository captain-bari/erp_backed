...
func login(w http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		log.Errorf("Login : RequestUnmarshallError: %s", err.Error())
		WriteCustomErrorResp(w, req, "", RequestUnmarshallError)
		return
	}

	log.Debugf("login: Request[%+v]", loginReq)

	notFound, name, role, err := auth.AuthenticateUser(loginReq.UserID, loginReq.UserHash)
	if err != nil {
		log.Errorf("Login : auth.AuthenticateUser: %s", err.Error())
	...
	if err != nil {
		log.Errorf("Login|GenerateJWT : ERROR:[%s]", err.Error())
...
func getMaterials(w http.ResponseWriter, req *http.Request) {
	...
	if tokkenStr == "" {
		log.Errorf("REST: Tokken not found")
		WriteCustomErrorResp(w, req, "", TokkenNotFound)
		return
	}
	valid, _, err := auth.CheckJwt(tokkenStr)
	if err != nil {
		log.Errorf("REST|AUTH|CheckJwt: Error while checking tokken [%s]", err.Error())
		WriteCustomErrorResp(w, req, "", AuthcontextNotFound)
		return
	}

	if !valid {
		log.Infof("REST|AUTH|CheckJwt: Invalid tokken:[%s]", tokkenStr)
		WriteCustomErrorResp(w, req, "", AuthcontextNotFound)
		return
	}
	...
	if err != nil {
		log.Errorf("REST|POSTGRES|GetMaterialsDB: ERROR[%s]", err.Error())
